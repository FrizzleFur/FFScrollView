...
- (void)adjustFrame
{
    ...
    CGFloat imageWidth  = bannerWidth;
    CGFloat imageHeight = bannerHeight;
    ...
    CGRect imageFrame = CGRectMake(0, 0, boundsWidth, boundsWidth * imageHeight / imageWidth);
    // 内容尺寸
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)gesture {
    if (gesture.state == UIGestureRecognizerStateBegan) {
        // 通知代理
        if ([self.photoViewDelegate respondsToSelector:@selector(photoViewDidLongPress:)]) {
            [self.photoViewDelegate photoViewDidLongPress:self];
        }
    }
}
