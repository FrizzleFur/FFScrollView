...
{
    [super viewDidLoad];
    self.automaticallyAdjustsScrollViewInsets = false;//navi bar的insets
...
//    [self.view addSubview:_toolbar];
    ...
    _pageControl = [[KZPageControl alloc] init];
    _pageControl.backgroundColor = [UIColor clearColor];
    _pageControl.pageIndicatorImage = [UIImage imageNamed:@"pageIndicator"];
    _pageControl.currentPageIndicatorImage = [UIImage imageNamed:@"currentPageIndicator"];
    _pageControl.indicatorMargin = 2.0;
    _pageControl.indicatorDiameter = 3.0;
    _pageControl.userInteractionEnabled = false;
    [self.view addSubview:_pageControl];
    
    _pageControl.frame = CGRectMake(0, self.view.height - _pageControl.indicatorDiameter - 20, self.view.width, _pageControl.indicatorDiameter);
    _pageControl.numberOfPages = _photos.count;
    _pageControl.hidden = (_photos.count <= 1);
...
- (void)createScrollView
{
    ...
    _photoScrollView.showsHorizontalScrollIndicator = false;
    _photoScrollView.showsVerticalScrollIndicator = false;
...
- (void)photoViewDidLongPress:(MJPhotoView *)photoView{
    if ([self.delegate respondsToSelector:@selector(didLongPressGestureInPhoto:)]) {
        [self.delegate didLongPressGestureInPhoto:photoView.photo.image];
    }
}
...
- (void)loadImageNearIndex:(int)index
{
    if (index > 0) {
        ...
        [SDWebImageManager downloadImageWithURL:photo.url];
    }
